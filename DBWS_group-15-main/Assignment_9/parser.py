...
def parser_error_log(filename):
    with open('./data/' + filename) as f, open('./data/' + 'error.csv','w') as csvf:
        log = f.read()
        regexp = r'\[([A-Za-z]{3} [A-Za-z]{3} \d{2} \d{2}:\d{2}:\d{2}).*(\d{4})\] \[(.*?)\] \[(.*?)\] \[client(.*?)\] [A-Z]{3} [A-Za-z]{6}: (.*?)\bin\b(.*(/~tpashakulo).*)'
        lists = re.findall(regexp,log)
        #creating csv object file
        writer = csv.writer(csvf)
...
if __name__ == '__main__':
    dict_of_pages = {
        'index': '/~tpashakulo',
        'login': '/~tpashakulo/login_form.html',
        'maintenance': '/~tpashakulo/maintance.html'

        }
    day='17' #Nov 17 as last thrusday was most traffic
